colpick <- function(design, q, all=FALSE, 
  ...
  if (!is.null(estimable)){
    ...
    if (p==0) map <- 1:n else
    map <- mapcalc(estimable, n, N, select.catlg=select.catlg, method=method, sort=sort)[[1]]
  ...
  while (i <= nr){
    ...
    if (all(colSums(X) > 0) || all0){ 
      ...
      if (is.null(estimable))
        ...
        if (all(estimchar %in% clearcur)) success <- TRUE
        if (success && !all) 
    ...
    if (length(clearcur) == min(phimax(n,q), nclear2fis) && !all){
      ...
      ## otherwise was already returned before
    } else{
      if (is.null(estimable)){
